...
delay_1 = int(input('Minimum time delay between entry additions (in seconds): '))
delay_2 = int(input('Maximum time delay between entry additions (in seconds): '))
# database = client[db_name]
...
# with open('testDB8_dict.csv', mode='w') as csv_file:
#     headers = ['name', 'bday', 'created', 'id']
#     writer = csv.DictWriter(csv_file, fieldnames=headers)

# writer.writerow(headers)
# writer.writeheader()
print(f'Adding {num_to_add} new items to {collection_name} table. With delay: min{delay_1} - max{delay_2}\n')
